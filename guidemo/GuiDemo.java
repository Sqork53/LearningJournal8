...
public class GuiDemo extends JFrame{
	...
	public GuiDemo() {
		...
		drawPanel.getTextItem().setText(
				"Java is a hard language to learn\n" +
				"      Everyday You learn something new,\n" +
				"You never stop learning,\n" +
				"      This will not be the last time I open ecliplse."
			);
		drawPanel.getTextItem().setFontSize(36);
		drawPanel.getTextItem().setJustify(TextItem.LEFT);
		drawPanel.setBackgroundImage(Util.getImageResource("resources/images/MyBackGround.jpg"));
		...
		setJMenuBar(menuBar);
		JToolBar tempTOOL = makeToolbar();
		content.add(tempTOOL, BorderLayout.NORTH);
		// Set the size of the window and its position.
		//this is where we create the menu and add it as well. 
		//Also we created the new toolBar and then we added it. 
    ...
    private class ChooseBackgroundAction extends AbstractAction {
    	...
    	ChooseBackgroundAction(String text) {
    		...
    		if (text.equals("Color...")) {
    			BufferedImage colorBuffer = new BufferedImage(32, 32,BufferedImage.TYPE_INT_RGB);
    					Graphics backroundColor = colorBuffer.createGraphics();
    					backroundColor.setColor(Color.RED);
    					backroundColor.fillRect(0, 0, 32, 32);
    					backroundColor.setColor(Color.GREEN);
    					backroundColor.fillRect(10, 0, 32, 32);
    					backroundColor.setColor(Color.BLUE);
    					backroundColor.fillRect(20, 0, 32, 32);
    					//I played around with the seetings to get like the full color and this was as close as I could get
    					backroundColor.dispose();//To be honest IDK why this is here but it is there in the IConSupport class
    					putValue(Action.SMALL_ICON, new ImageIcon(colorBuffer));
    					putValue(Action.SHORT_DESCRIPTION,
    					"To set the background color."); // tooltip   		
    		}
    		else if (text.equals("Custom...")) {
    			putValue(Action.SMALL_ICON, Util.iconFromResource("resources/action_button/fileopen.png"));
    			putValue(Action.SHORT_DESCRIPTION, "<html>Select an image file<br>to use as the background.</html>");
    		//Adding the icon to the custom on the jbar
		}
    		else
    ...
    //This method will be used to create the toolbar that is north on the window.
